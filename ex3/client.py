...
class FTPClient:
    ...
    def rename_file(self):
        ...
        choice = input("Select file to rename (number or name): ").strip()
        try:
            file_index = int(choice) - 1
            if 0 <= file_index < len(files):
        ...
        response = self.send_command({'command': 'rename_file', 'old_name': old_name, 'new_name': new_name})
        if response['status'] == 'success':
            print(f"✓ {response['message']}")
        else:
            print(f"✗ {response['message']}")
    ...
    def read_file(self):
        ...
        choice = input("Select file to read (number or name): ").strip()
        try:
            file_index=int(choice)-1
            if 0<=file_index<len(files):
                filename = files[file_index]
    ...
    def download(self):
        ...
        choice = input("Select file to download (number or name):").strip()
        try:
            file_index = int(choice)-1
            if 0 <= file_index < len(files):
                filename = files[file_index]
    ...
    def edit_file(self):
        ...
        choice = input("Select file to edit (number or name): ").strip()
        try:
            file_index = int(choice) - 1
            if 0 <= file_index < len(files):
                filename = files[file_index]
    ...
    def see_file_operation_history(self):
        ...
        choice = input("Select file to see history (number or name): ").strip()
        try:
            file_index = int(choice)-1
            if 0<=file_index < len(files):
                filename = files[file_index]
