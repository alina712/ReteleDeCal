...
def record_operation(filename, operation, user):
    timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    with history_lock:
        if filename not in file_history:
            file_history[filename] = []
        file_history[filename].append(f"[{timestamp}] {operation} by {user}")
        ...
        while True:
            ...
            try:
                ...
                # File operations
                    ...
                    with open(filepath, 'w') as f:
                        f.write(content)
                    record_operation(filename, 'uploaded', current_user)
                    ...
                    if not os.path.exists(old_path):
                        ...
                        record_operation(new_name, f'renamed from {old_name}', current_user)
                    ...
                    if not os.path.exists(filepath):
                        ...
                        with open(filepath, 'w') as f:
                            f.write(content)
                        record_operation(filename, 'edited', current_user)
                    ...
                    if not history:
                        message = f'No operation history for {filename}'
                    else:
                        message = '\n'.join(history)
